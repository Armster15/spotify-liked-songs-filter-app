...
export const LoadingProgress: React.FC<LoadingProgressProps> = ({ loadingState }) => {
  ...
  const getProgressPercentage = () => {
    ...
    return Math.min(100, (loadingState.progress.current / loadingState.progress.total) * 100);
  };
  ...
  return (
    <div className="container">
      ...
      >
        ...
        <h2 style={{ color: '#1db954', marginBottom: '15px', fontSize: '24px' }}>
        ...
        <div style={{ color: '#b3b3b3', fontSize: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span>🎵</span>
              <span style={{ opacity: loadingState.phase === 'fetching_songs' ? 1 : 0.5 }}>
                Fetch Songs
              </span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span>🎭</span>
              <span style={{ opacity: loadingState.phase === 'fetching_genres' ? 1 : 0.5 }}>
                Get Genres
              </span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span>⚙️</span>
              <span style={{ opacity: loadingState.phase === 'processing' ? 1 : 0.5 }}>
                Process
              </span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span>✅</span>
              <span style={{ opacity: loadingState.phase === 'complete' ? 1 : 0.5 }}>
                Complete
              </span>
            </div>
          </div>
        ...
        {loadingState.phase === 'fetching_songs' && (
          <div style={{ marginTop: '20px', color: '#b3b3b3', fontSize: '12px' }}>
        ...
        {loadingState.phase === 'fetching_genres' && (
          <div style={{ marginTop: '20px', color: '#b3b3b3', fontSize: '12px' }}>
            <p>🎨 Analyzing artist genres to categorize your music</p>
            <p>⚡ Using batch requests for optimal performance</p>
          </div>
        )}
      </div>
    </div>
